...
{
    Dictionary<char, int> occurences = new(); int jokers = 0;
    foreach (char c in hand)
        if (c == 'J') jokers++;
        else if (occurences.ContainsKey(c)) occurences[c]++;
        else occurences.Add(c, 1);

    int type = -1;
    if (jokers == 5) type = 6; // ONLY 5 of a kind
    else if (jokers == 4) type = 6; // ONLY 5 of a kind
    else if (jokers == 3)
    {
        if (occurences.ContainsValue(2)) type = 6; // 5 of a kind
        else type = 5; // only 4 of a kind
    }
    else if (jokers == 2)
    {
        if (occurences.ContainsValue(3)) type = 6; // 5 of a kind
        else if (occurences.ContainsValue(2)) type = 5; // 4 of a kind
        else type = 3; // 3 of a kind
    }
    else if (jokers == 1)
    {
        if (occurences.ContainsValue(4)) type = 6; // 5 of a kind
        else if (occurences.ContainsValue(3)) type = 5; // 4 of a kind
        else if (occurences.ContainsValue(2) && !occurences.ContainsValue(1)) type = 4; // full house
        else if (occurences.ContainsValue(2)) type = 3; // 3 of a kind
        else type = 1; // 1 pair
    }
    else
    {
