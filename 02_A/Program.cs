...
foreach (string line in data)
{
    ...
    // if max for each is lower than possible maximum, then we're good and add the id. (otherwise, we don't)
    if (lineRed <= MAX_RED && lineGreen <= MAX_GREEN && lineBlue <= MAX_BLUE)
        answer += id;
}
